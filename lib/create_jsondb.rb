...
# argument: URL.php to get JSON format output from mysql of sra.dbcls.jp, get Submission id - PMID pair and its related metadata
	...
	nakamidono.each do |nakami|
		irumono = [] # individual entry: pmid, article_title, journal, date, sra_id, pm_abstract, pm_author (array includes content only, not hash)
		nakami.each do |naka|
	...
	begin
		...
		if !File.exist?(expdono)
			...
			exp_xml.css("EXPERIMENT").each do |p|
				...
				platform = (re_parse/"INSTRUMENT_MODEL").inner_text.chomp
				if platform.empty?
					peko.push("no description for platform")
				else
					peko.push(platform)
				end
				
				space = (re_parse/"SEQUENCE_SPACE").inner_text.chomp
				if space.empty?
					peko.push("no description for sequence space")
				else
					peko.push(space)
				end
				...
				if !File.exist?(rundono)
					peko.push(["no metadata for sequence run"])
				else
					...
					run_xml.css("RUN").each do |n|
						...
						exp_id_for_run = (re_parse/"EXPERIMENT_REF").attr("accession").to_s
						
						if exp_id == exp_id_for_run
							
							dra_page = open("http://trace.ddbj.nig.ac.jp/DRASearch/run?acc=#{run}")
							spotnum = dra_page.scan(/Number of spots<\/td><td><span id=\"number_of_spot\">(.*)<\/span><\/td><\/tr>/).flatten.join("").to_i
							basenum = dra_page.scan(/Number of bases<\/td><td>(.*)<\/td><\/tr>/).flatten.join("").to_i
							read_length = basenum / spotnum
							
							neko.push(run_id)
							neko.push(spotnum)
							neko.push(read_length)
				...
				if sample_id.empty?
					peko.push("no metadata for sample")
				else
					peko.push(sample_id)
				end
